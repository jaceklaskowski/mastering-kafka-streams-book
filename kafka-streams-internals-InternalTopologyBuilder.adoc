...
NOTE: `buildProcessorNode` is used exclusively when `InternalTopologyBuilder` is requested to <<build, build a ProcessorTopology>> (with <<kafka-streams-ProcessorNodeFactory.adoc#, ProcessorNodeFactories>>).
...
`addSource` registers a <<kafka-streams-SourceNodeFactory.adoc#, SourceNodeFactory>> for the given topics (listed explicitly or using Java's https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/regex/Pattern.html[java.util.regex.Pattern]).

For every topic name in the given `topics`, `addSource`:

. <<validateTopicNotAlreadyRegistered, validateTopicNotAlreadyRegistered>>
. <<maybeAddToResetList, maybeAddToResetList>>
. Adds the topic name to the <<sourceTopicNames, sourceTopicNames>> internal registry
...
`addSource` requests <<nodeGrouper, QuickUnion of the names of node groups>> to link:kafka-streams-internals-QuickUnion.adoc#add[add] the `name`.
...
* <<kafka-streams-internals-GroupedTableOperationRepartitionNode.adoc#writeToTopology, GroupedTableOperationRepartitionNode>>, <<kafka-streams-internals-OptimizableRepartitionNode.adoc#writeToTopology, OptimizableRepartitionNode>>, <<kafka-streams-internals-StreamSourceNode.adoc#writeToTopology, StreamSourceNode>>, and <<kafka-streams-internals-TableSourceNode.adoc#writeToTopology, TableSourceNode>> are requested to `writeToTopology`
====
...
`addGlobalStore` creates a <<kafka-streams-ProcessorNodeFactory.adoc#, ProcessorNodeFactory>> with the given `processorName`, `sourceName` (as <<kafka-streams-ProcessorNodeFactory.adoc#predecessors, predecessors>>) and `stateUpdateSupplier` (as <<kafka-streams-ProcessorNodeFactory.adoc#supplier, supplier>>).
...
. Creates a <<kafka-streams-SourceNodeFactory.adoc#, SourceNodeFactory>> and registers it in <<nodeFactories, nodeFactories>> internal registry as `sourceName`
...
* `KStreamImplJoin` is requested to link:kafka-streams-internals-KStreamImplJoin.adoc#join[join]
