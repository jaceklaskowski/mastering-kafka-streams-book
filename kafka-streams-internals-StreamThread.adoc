...
[[pollTime]]
`StreamThread` uses the <<kafka-streams-properties.adoc#poll.ms, poll.ms>> configuration property (default: `100` ms) as the *polling interval* when requested to <<runOnce, poll records once and adding them to active stream tasks>>.
...
* With <<pollTime, pollTime>> in any other state but `PARTITIONS_ASSIGNED`
====
