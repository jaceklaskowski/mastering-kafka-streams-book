...
`StreamThread` uses a <<rebalanceListener, custom ConsumerRebalanceListener>> (that has access to the <<taskManager, TaskManager>>) when subscribing to source topics (when requested to <<runLoop, run the main record processing loop>> and <<enforceRebalance, enforceRebalance>>).
