...
`StandbyTaskCreator` is <<creating-instance, created>> exclusively when `StreamThread` is <<kafka-streams-internals-StreamThread.adoc#create, created>> (for the only purpose of <<kafka-streams-internals-TaskManager.adoc#standbyTaskCreator, creating a TaskManager>>).
