...
`StandbyTaskCreator` is a concrete <<kafka-streams-internals-AbstractTaskCreator.adoc, AbstractTaskCreator>> of <<createTask, standby tasks>>.

`StandbyTaskCreator` is <<creating-instance, created>> exclusively when `StreamThread` is link:kafka-streams-StreamThread.adoc#create[created] (just to create a link:kafka-streams-internals-TaskManager.adoc#standbyTaskCreator[TaskManager]).
...
NOTE: `createTask` is part of link:kafka-streams-internals-AbstractTaskCreator.adoc#createTask[AbstractTaskCreator Contract] to create concrete link:kafka-streams-internals-AbstractTask.adoc[stream processor tasks].
