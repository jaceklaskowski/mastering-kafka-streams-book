...
`TaskManager` manages <<active, active>> and <<standby, standby>> collections of tasks. `TaskManager` adds a new active task when `RebalanceListener` handles link:kafka-streams-StreamThread-RebalanceListener.adoc#onPartitionsAssigned[onPartitionsAssigned] (and requests to <<createTasks, create processor tasks for assigned topic partitions>> that in turn <<addStreamTasks, adds new stream tasks>>).

`TaskManager` is <<creating-instance, created>> when `StreamThread` is link:kafka-streams-StreamThread.adoc#create[created] (with a new <<active, AssignedStreamsTasks>> and <<standby, AssignedStandbyTasks>> for <<active, active>> and <<standby, standby>> collections of tasks, respectively).

.Creating TaskManager
image::images/kafka-streams-TaskManager.png[align="center"]

`TaskManager` uses Kafka's <<adminClient, AdminClient>> for <<maybePurgeCommitedRecords, maybePurgeCommitedRecords>> and for `StreamsPartitionAssignor` to link:kafka-streams-StreamsPartitionAssignor.adoc#configure[configure] itself (and create the internal link:kafka-streams-StreamsPartitionAssignor.adoc#internalTopicManager[InternalTopicManager]).
...
NOTE: `setAssignmentMetadata` is used exclusively when `StreamsPartitionAssignor` is requested to link:kafka-streams-StreamsPartitionAssignor.adoc#onAssignment[handle assignment from the leader].
