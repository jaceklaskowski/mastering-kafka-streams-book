...
NOTE: `register` is part of link:kafka-streams-internals-ChangelogReader.adoc#register[ChangelogReader Contract] to register a <<kafka-streams-StateRestorer.adoc#, StateRestorer>> (for a changelog partition).
...
| needsInitializing
a| [[needsInitializing]] Changelog partitions (of <<kafka-streams-StateRestorer.adoc#, StateRestorers>>) that need initializing (`Set<TopicPartition>`)
...
| stateRestorers
a| [[stateRestorers]] <<kafka-streams-StateRestorer.adoc#, StateRestorers>> per changelog partition (`Map<TopicPartition, StateRestorer>`)
