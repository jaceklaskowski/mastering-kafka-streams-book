...
`RocksDbKeyValueBytesStoreSupplier` is <<creating-instance, created>> when `Stores` utility is used to create persistent <<kafka-streams-StateStore-KeyValueStore.adoc#, KeyValueStores>>:
...
`get` gives the following <<kafka-streams-StateStore-KeyValueStore.adoc#, KeyValueStores>>:
