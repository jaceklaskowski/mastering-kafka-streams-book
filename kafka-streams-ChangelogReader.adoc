...
Used exclusively when `TaskManager` is requested to <<kafka-streams-TaskManager.adoc#createTasks, create processor tasks for assigned topic partitions>> (when `RebalanceListener` handles <<kafka-streams-StreamThread-RebalanceListener.adoc#onPartitionsAssigned, onPartitionsAssigned>>)
