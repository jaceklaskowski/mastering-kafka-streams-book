...
`configure` uses the given `configs` to find the current <<taskManager, TaskManager>> using `__task.manager.instance__` internal property. If not found or of a different type than `TaskManager`, `configure` throws a `KafkaException`:
...
`configure` uses the given `configs` to find the current <<assignmentErrorCode, assignmentErrorCode>> using `__assignment.error.code__` internal property. If not found or of a different type than `AtomicInteger`, `configure` throws a `KafkaException`:
