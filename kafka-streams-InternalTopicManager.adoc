...
`InternalTopicManager` is <<creating-instance, created>> exclusively for the <<kafka-streams-StreamsPartitionAssignor.adoc#internalTopicManager, StreamsPartitionAssignor>> so it can <<makeReady, make repartition and state changelog internal topics ready>>.
...
NOTE: `makeReady` is used exclusively when `StreamsPartitionAssignor` is requested to <<kafka-streams-StreamsPartitionAssignor.adoc#prepareTopic, prepareTopic>> (when requested to <<kafka-streams-StreamsPartitionAssignor.adoc#assign, perform group assignment>>).
