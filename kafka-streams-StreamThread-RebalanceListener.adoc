...
`RebalanceListener` is a Apache Kafka https://kafka.apache.org/10/javadoc/org/apache/kafka/clients/consumer/ConsumerRebalanceListener.html[ConsumerRebalanceListener] that listens to changes to the partitions assigned to a single instance of a Kafka Streams application.
...
At the completion of a successful partition re-assignment (i.e. <<onPartitionsAssigned, onPartitionsAssigned>> event) `RebalanceListener` requests <<taskManager, TaskManager>> for link:kafka-streams-TaskManager.adoc#createTasks[creating processor tasks for assigned topic partitions].

At the start of a rebalance operation (i.e. <<onPartitionsRevoked, onPartitionsRevoked>> event) `RebalanceListener` requests <<taskManager, TaskManager>> for link:kafka-streams-TaskManager.adoc#suspendTasksAndState[suspending all stream tasks and state].
