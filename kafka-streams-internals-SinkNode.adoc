...
`SinkNode` is <<creating-instance, created>> exclusively when `SinkNodeFactory` is requested to link:kafka-streams-internals-InternalTopologyBuilder-SinkNodeFactory.adoc#build[build a processor node] (when `InternalTopologyBuilder` is requested to link:kafka-streams-internals-InternalTopologyBuilder.adoc#build[build a processor task topology] for specified node groups).
