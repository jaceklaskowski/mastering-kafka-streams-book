...
`StreamsBuilder` provides the <<operators, high-level stream processing DSL>> to <<build, build a processor topology>> (with sources, processors, sinks and state stores).

TIP: Consider <<kafka-streams-scala.adoc#, Scala API for Kafka Streams>> to make your Kafka Streams development more pleasant if Scala is your programming language.
