...
`SourceNode` is <<creating-instance, created>> exclusively when `SourceNodeFactory` is requested to link:kafka-streams-SourceNodeFactory.adoc#build[build a processor node] (when...FIXME)
