...
`GlobalStateUpdateTask` is <<creating-instance, created>> exclusively when `GlobalStreamThread` is requested to link:kafka-streams-internals-GlobalStreamThread.adoc#initialize[initialize] (and creates a link:kafka-streams-internals-StateConsumer.adoc#stateMaintainer[StateConsumer]).
...
`update` looks up the link:kafka-streams-internals-RecordDeserializer.adoc[RecordDeserializer] for the topic (of the record) in the <<deserializers, deserializers>> internal registry.

NOTE: `RecordDeserializer` is the metadata of a link:kafka-streams-internals-RecordDeserializer.adoc#sourceNode[SourceNode] that link:kafka-streams-internals-RecordDeserializer.adoc#deserialize[knows how to deserialize a raw record] in a given `ProcessorContext`.
...
1. Requests <<processorContext, InternalProcessorContext>> to link:kafka-streams-internals-InternalProcessorContext.adoc#setCurrentNode[use the source node] of the link:kafka-streams-internals-RecordDeserializer.adoc#sourceNode[RecordDeserializer]

1. Requests the link:kafka-streams-internals-RecordDeserializer.adoc#sourceNode[SourceNode] (of the deserialized record) to link:kafka-streams-internals-SourceNode.adoc#process[process] the deserialized record
...
`flushState` requests <<stateMgr, GlobalStateManager>> to link:kafka-streams-internals-StateManager.adoc#flush[flush] followed by link:kafka-streams-internals-Checkpointable.adoc#checkpoint[checkpointing] the <<offsets, offsets>>.
...
* [[stateMgr]] link:kafka-streams-internals-GlobalStateManager.adoc[GlobalStateManager]
