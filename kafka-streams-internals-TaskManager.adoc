...
`cachedTasksIds` collects the <<kafka-streams-TaskId.adoc#parse, TaskIds>> (from the names of the task directories) that correspond to task directories with <<kafka-streams-internals-AbstractStateManager.adoc#CHECKPOINT_FILE_NAME, .checkpoint>> file.
