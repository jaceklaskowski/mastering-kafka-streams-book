...
`StreamsConfig` is used to reference the <<properties, properties names>> (e.g. to avoid any typos or for type safety).
...
NOTE: `InternalStreamsConfig` is an extension of `StreamsConfig` that allows for specifying custom <<props, properties>> with the <<doLog, doLog>> flag off.
