...
`SessionStoreBuilder` is an link:kafka-streams-AbstractStoreBuilder.adoc[AbstractStoreBuilder] that <<build, builds a MeteredSessionStore>> (possibly with <<maybeWrapCaching, caching>> and <<maybeWrapLogging, logging>>).
