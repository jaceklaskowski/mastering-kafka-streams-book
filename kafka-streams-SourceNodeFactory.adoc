...
Internally, `build` looks up the link:kafka-streams-NodeFactory.adoc#name[node name] in the link:kafka-streams-internals-InternalTopologyBuilder.adoc#nodeToSourceTopics[nodeToSourceTopics] internal registry to find the topic names (the node is attached to).
