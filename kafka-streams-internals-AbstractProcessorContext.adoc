...
NOTE: `register` is part of the <<kafka-streams-ProcessorContext.adoc#register, ProcessorContext Contract>> to register a <<kafka-streams-StateStore.adoc#, state store>> (and a <<kafka-streams-StateRestoreCallback.adoc#, StateRestoreCallback>>).

`register` simply requests the <<stateManager, StateManager>> to <<kafka-streams-internals-StateManager.adoc#register, register the state store>> (and the <<kafka-streams-StateRestoreCallback.adoc#, StateRestoreCallback>>).

When <<initialized, initialized>> already, `register` throws an `IllegalStateException`:
