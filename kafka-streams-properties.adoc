...
| application.id
| (empty)
| [[application.id]] *Application ID* that is the required identifier of a Kafka Streams stream processing application
...
Used exclusively when `StreamPartitionAssignor` is requested to <<kafka-streams-StreamsPartitionAssignor.adoc#configure, configure itself>>.
