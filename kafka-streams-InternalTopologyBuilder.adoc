...
1. link:kafka-streams-ProcessorNodeFactory.adoc#addStateStore[Adds] the `name` (of a state store) to the `ProcessorNodeFactory`
...
=== [[makeNodeGroups]] `makeNodeGroups` Internal Method
...
`makeNodeGroups`...FIXME

NOTE: `makeNodeGroups` is used when `InternalTopologyBuilder` is requested to <<describe, describe>>, <<nodeGroups-accessor, nodeGroups>>
...
`nodeGroups` gives <<nodeGroups, nodes by id>>.

If <<nodeGroups, nodeGroups>> is empty, `nodeGroups` <<makeNodeGroups, makeNodeGroups>> that become the current <<nodeGroups, nodeGroups>>.
...
=== [[globalNodeGroups]] `globalNodeGroups` Internal Method
...
`globalNodeGroups` takes <<nodeGroups-accessor, node groups>> and <<isGlobalSource, filters out non-global node groups>> (that leaves global node groups only).

NOTE: `globalNodeGroups` gives node groups with at least one global node group.
...
=== [[isGlobalSource]] Checking If Node Name is of Global Source -- `isGlobalSource` Internal Method
...
`isGlobalSource` takes the `NodeFactory` for the given `nodeName` (from <<nodeFactories, nodeFactories>>).

`isGlobalSource` is positive (i.e. `true`) when all the following hold:

* `nodeName` is a link:kafka-streams-SourceNodeFactory.adoc[SourceNodeFactory]

* `SourceNodeFactory` has exactly one topic in link:kafka-streams-SourceNodeFactory.adoc#topics[topics]
