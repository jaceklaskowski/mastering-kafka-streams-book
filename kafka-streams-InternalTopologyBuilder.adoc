...
a| link:kafka-streams-NodeFactory.adoc[NodeFactories] by name
...
`addGlobalStore` then does the following housekeeping:
...
`globalNodeGroups` takes <<nodeGroups-accessor, node groups>> and <<isGlobalSource, filters out non-global node groups>>.
...
=== [[isGlobalSource]] `isGlobalSource` Internal Method
