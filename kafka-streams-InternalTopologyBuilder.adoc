...
a| [[storeToChangelogTopic]] Topic names by the names of state stores
...
`addStateStore` creates a <<kafka-streams-StoreBuilderFactory.adoc#creating-instance, StoreBuilderFactory>> and adds it to the <<stateFactories, stateFactories>> internal registry.
...
* `KStreamImpl` is requested to <<kafka-streams-KStreamImpl.adoc#createReparitionedSource, createReparitionedSource>>
