...
`KStreamTransformValues` is <<creating-instance, created>> exclusively when `KStreamImpl` is requested to link:kafka-streams-KStreamImpl.adoc#transformValues[transform values with optional state] (aka _stateful record-by-record value transformation_).
