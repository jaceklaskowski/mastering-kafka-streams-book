...
=== [[mapValues]] Registering Node Processor with KStreamMapValues Processor Supplier (with KSTREAM-MAPVALUES Prefix) -- `mapValues` Operator
...
`to` link:kafka-streams-InternalTopologyBuilder.adoc#addProcessor[registers a new sink node] under the name.
