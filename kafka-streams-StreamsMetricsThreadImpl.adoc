...
`StreamsMetricsThreadImpl` is <<creating-instance, created>> for a link:kafka-streams-internals-StreamThread.adoc#create[StreamThread] (when `KafkaStreams` is link:kafka-streams-KafkaStreams.adoc#creating-instance[created]).
